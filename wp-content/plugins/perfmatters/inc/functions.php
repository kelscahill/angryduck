...
if(!empty($perfmatters_ga['enable_local_ga']) && $perfmatters_ga['enable_local_ga'] == "1") {
	...
	if(empty($perfmatters_ga['script_type']) || $perfmatters_ga['script_type'] == 'gtagv4') {
...
function perfmatters_update_ga() {
	...
	elseif($perfmatters_ga['script_type'] == 'gtagv4') {
		if(!empty($perfmatters_ga['tracking_id'])) {
			$remote_file = 'https://www.googletagmanager.com/gtag/js?id=' . $perfmatters_ga['tracking_id'];
			$local_file = WP_CONTENT_DIR . '/uploads/perfmatters/gtagv4.js';
		}
	}

	if(!empty($remote_file) && !empty($local_file)) {

		$file = wp_remote_get($remote_file);

		if(is_wp_error($file)) {
	    	return $file->get_error_code() . ': ' . $file->get_error_message();
	    }

	    if(!is_dir(WP_CONTENT_DIR . '/uploads/perfmatters/')) {
	    	wp_mkdir_p(WP_CONTENT_DIR . '/uploads/perfmatters/');
	    }

	   	file_put_contents($local_file, $file['body']);
	}
}
...
function perfmatters_print_ga() {
	...
	elseif($perfmatters_ga['script_type'] == 'gtagv4') {
		echo '<!-- Local Analytics generated with perfmatters. -->';
    	echo '<script src="' . content_url() . '/uploads/perfmatters/gtagv4.js"></script>'; 
    	echo '<script>window.dataLayer = window.dataLayer || [];function gtag(){dataLayer.push(arguments);}gtag("js", new Date());gtag("config", "' . $perfmatters_ga['tracking_id'] . '");</script>';
	}
...
function perfmatters_update_option_perfmatters_ga($old_value, $new_value) {
	...
	if($new_value['script_type'] != $old_value['script_type'] && (empty($new_value['script_type']) || $new_value['script_type'] == 'gtagv4')) {
		$update_flag = true;
	}

	if(!empty($new_value['tracking_id']) && $new_value['tracking_id'] != $old_value['tracking_id'] && $new_value['script_type'] == 'gtagv4') {
		$update_flag = true;
	}
