    ...
    <?php if ( 'yes' === WPay::get_overall_auto_charge_status( $payment_plans ) ) : ?>
        <p>
            <em>
                ...
                printf(
                    /* translators: %1$s = opening <strong> tag; %2$s = closing </strong> tag */
                    esc_html__( '%1$sAuto-charge%2$s means Stripe will automatically debit the customer\'s saved payment method(e.g. credit card, debit card, ACH) without requiring manual action from the customer for each payment.', 'woocommerce-wholesale-payments' ),
                    '<strong>',
                    '</strong>'
                );
                ?>
